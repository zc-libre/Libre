      ...
      <ul class="timeline timeline-centered">
        ...
        >
          ...
          >
            ...
            <div class="timeline-content">
              <router-link
                :to="{ name: 'article', params: { articleId: post.id } }"
              >
                <h3 class="timeline-title">{{ post.articleName }}</h3>
              </router-link>
              <p>
                {{ post.summary }}
              </p>
...
export default defineComponent({
  ...
  setup() {
    ...
    const {t} = useI18n()
    const archives = ref(new Archives().records)
    ...
    const fetchData = async () => {
      const data = await postStore.fetchArchives(pagination.value)
